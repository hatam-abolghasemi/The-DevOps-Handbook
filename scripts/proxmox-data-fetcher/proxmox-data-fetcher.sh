...
# data processing
rm qemu-resources.json node-resources.json storage-resources.json
jq '.data | map(select(.type == "qemu")) | {data: .}' cluster-resources.json >> qemu-resources.json
jq '.data | map(select(.type == "node")) | {data: .}' cluster-resources.json >> node-resources.json
jq '.data | map(select(.type == "storage")) | {data: .}' cluster-resources.json >> storage-resources.json
...
while IFS= read -r node_name; do
    NODE_RRD_STATS_URL="$CLUSTER_URL""nodes/$node_name/rrddata?timeframe=hour&cf=AVERAGE"
...
while IFS= read -r node_name; do
    NODE_STATUS_URL="$CLUSTER_URL""nodes/$node_name/status"
    if [ -f "$node_name/node-status.json" ]; then
        rm "$node_name/node-status.json"
    fi
    curl -s -H "Authorization: PVEAPIToken=$API_TOKEN" "$NODE_STATUS_URL" | jq . >> "$node_name/node-status.json"
done < "node-names.tmp"
...
while IFS= read -r node_name; do
    NODE_TIME_URL="$CLUSTER_URL""nodes/$node_name/time"
    if [ -f "$node_name/node-time.json" ]; then
        rm "$node_name/node-time.json"
    fi
    curl -s -H "Authorization: PVEAPIToken=$API_TOKEN" "$NODE_TIME_URL" | jq . >> "$node_name/node-time.json"
done < "node-names.tmp"
...
while IFS= read -r qemu_name; do
    NODE_QEMU_STATUS_URL="$CLUSTER_URL""nodes/$qemu_name/status/current"
    ...
    curl -s -H "Authorization: PVEAPIToken=$API_TOKEN" "$NODE_QEMU_STATUS_URL" | jq . >> "$node_name/$pure_qemu-status.json"
...
while IFS= read -r node_name; do
    NODE_NETWORK_URL="$CLUSTER_URL""nodes/$node_name/network"
    if [ -f "$node_name/node-network-list.json" ]; then
        rm "$node_name/node-network-list.json"
    fi
    curl -s -H "Authorization: PVEAPIToken=$API_TOKEN" "$NODE_NETWORK_URL" | jq . >> "$node_name/node-network-list.json"
done < "node-names.tmp"
...
while IFS= read -r node_name; do
    NODE_STORAGE_STATUS_URL="$CLUSTER_URL""nodes/$node_name/storage"
    if [ -f "$node_name/node-storage-status.json" ]; then
        rm "$node_name/node-storage-status.json"
    fi
    curl -s -H "Authorization: PVEAPIToken=$API_TOKEN" "$NODE_STORAGE_STATUS_URL" | jq . >> "$node_name/node-storage-status.json"
done < "node-names.tmp"
...
# cleanup
rm node-names.tmp node-resources.tmp cluster-resources.json
